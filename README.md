...
Дополнительно дашборд умеет следить за файлами правил ИИ в локальной или сетевой папке, например для роли `Developer`, и подсвечивать возможные противоречия, ослабления ограничений и лазейки.
...
- `configure_rules_monitor` - настроить папку с правилами ИИ и имя роли.
...
- путь к папке с правилами, включая UNC-путь вида `\\PC\share\rules`.

Сервер перечитывает файлы `.md`, `.txt`, `.json`, `.yaml`, `.yml`, сравнивает их с предыдущим снимком и показывает:
...
Если путь сетевой, служба Windows должна иметь права чтения этой сетевой папки. По умолчанию `AuditorDashboard` работает как `LocalSystem`, а такая учетная запись не всегда имеет доступ к шарам другого ПК в рабочей группе.
